...
for name in names :
    with open(rospack.get_path('ens_vision')+f'/tests/{name}_aruco.yaml') as file:
...
ax3.set_title('Crosstrack error (meter)')
ct_dict = ax3.boxplot(crosstracks, labels=labels, showmeans=True, meanline=True)
ax4.set_title('Yaw error (radian)')
yaw_dict = ax4.boxplot(yaws, labels = labels,showfliers=True, showmeans=True, meanline=True)     